...
public:
    ...
    void insert(const T& value) {
        ...
        while (n > 0 && !(data_[parent] > data_[n])) {
    ...
    void remove() {
        ...
        while ((left < data_.size() && !(data_[n] > data_[left])) || (right < data_.size() && !(data_[n] > data_[right]))) {
...
int main() {
    ...
    my::binary_max_heap<int> heap;
    for (int i = 0; i < 20; ++i) {
        heap.insert(range(rng));
    }
